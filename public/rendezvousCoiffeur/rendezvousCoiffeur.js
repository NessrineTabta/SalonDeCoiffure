...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  let rendezVous = []; // Variable pour stocker les rendez-vous
  ...
  // Fonction pour afficher les rendez-vous pour la date sélectionnée
  function showAppointmentsForSelectedDate(date) {
    const selectedDate = date;
    const selectedDay = selectedDate.getDate();
    const selectedMonth = selectedDate.getMonth();
    const selectedYear = selectedDate.getFullYear();

    const appointments = rendezVous.filter((appointment) => {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        "Erreur lors de la récupération des rendez-vous du coiffeur"
      );
    }

    rendezVous = await response.json(); // Stocker les rendez-vous
